...
public class ShadowDrawableWrapper extends DrawableWrapper {
    ...
    private void drawShadow(@NonNull Canvas canvas) {
        ...
        float edgeShadowTop = -cornerRadius - shadowSize;
        float shadowOffset = cornerRadius;
        boolean drawHorizontalEdges = contentBounds.width() - 2 * shadowOffset > 0;
        boolean drawVerticalEdges = contentBounds.height() - 2 * shadowOffset > 0;

        float shadowOffsetTop = rawShadowSize - (rawShadowSize * SHADOW_TOP_SCALE);
        float shadowOffsetHorizontal = rawShadowSize - (rawShadowSize * SHADOW_HORIZ_SCALE);
        float shadowOffsetBottom = rawShadowSize - (rawShadowSize * SHADOW_BOTTOM_SCALE);
    ...
    private void buildComponents(@NonNull Rect bounds) {
        ...
        float verticalOffset = rawMaxShadowSize * SHADOW_MULTIPLIER;
        contentBounds.set(
    ...
    public float getMinWidth() {
        float content = 2 * Math.max(rawMaxShadowSize, cornerRadius + rawMaxShadowSize / 2);

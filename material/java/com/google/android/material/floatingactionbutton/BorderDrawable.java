...
class BorderDrawable extends Drawable {
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        float halfBorderWidth = paint.getStrokeWidth() / 2f;
        copyBounds(rect);
        rectF.set(rect);
    ...
    protected boolean onStateChange(int[] state) {
        if (borderTint != null) {
            int newColor = borderTint.getColorForState(state, currentBorderTintColor);
    ...
    private Shader createGradientShader() {
        ...
        float borderRatio = borderWidth / rect.height();

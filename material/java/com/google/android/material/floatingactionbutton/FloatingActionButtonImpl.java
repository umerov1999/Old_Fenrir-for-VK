...
class FloatingActionButtonImpl {
    ...
    void getPadding(@NonNull Rect rect) {
        int minPadding = ensureMinTouchTargetSize
                ? (minTouchTargetSize - view.getSizeDimension()) / 2
                : 0;

        float maxShadowSize = shadowPaddingEnabled ? (getElevation() + pressedTranslationZ) : 0;
        int hPadding = Math.max(minPadding, (int) Math.ceil(maxShadowSize));
        int vPadding = Math.max(minPadding, (int) Math.ceil(maxShadowSize * SHADOW_MULTIPLIER));
        rect.set(hPadding, vPadding, hPadding, vPadding);
    }

    void onPaddingUpdated(@NonNull Rect padding) {
        checkNotNull(contentBackground, "Didn't initialize content background");
        if (shouldAddPadding()) {
